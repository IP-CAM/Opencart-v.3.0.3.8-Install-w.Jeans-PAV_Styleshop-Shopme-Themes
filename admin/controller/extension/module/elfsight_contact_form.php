...
class Deactivate {
    ...
    public function __construct($data) {
        ...
        $this->app_slug = 'contact-form';
        $this->app_name = 'Contact Form';
    ...
    public function setDeactivateReasons() {
        return require ( (__DIR__) . '/contact-form-assets/reasons.php' );
    }
...
class ControllerExtensionModuleElfsightContactForm extends Controller {
	private $error = array();

	public $params = NULL;

    public $ELFSIGHT_EMBED_URL = 'https://apps.elfsight.com/embed/contact-form/?utm_source=portals&utm_medium=opencart&utm_campaign=contact-form&utm_content=sign-up';
	...
	public function index() {
		$this->load->language('extension/module/elfsight_contact_form');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('module_elfsight_contact_form', $this->request->post);
		...
		$data['action'] = $this->url->link('extension/module/elfsight_contact_form', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'token=' . $this->session->data['token'] . '&type=module', true);

		if (isset($this->request->post['module_elfsight_contact_form_status'])) {
			$data['module_elfsight_contact_form_status'] = $this->request->post['module_elfsight_contact_form_status'];
		} else {
			$data['module_elfsight_contact_form_status'] = $this->config->get('module_elfsight_contact_form_status');
		}

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');

		$this->response->setOutput($this->load->view('extension/module/elfsight_contact_form', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/elfsight_contact_form')) {
    ...
    public function setDeactivateReasons() {
        return require ( './controller/extension/module/contact-form-assets/reasons.php' );
    }
    ...
    public function uninstall() {
        ...
        include './controller/extension/module/contact-form-assets/popup.php';
    }
